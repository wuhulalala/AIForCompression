"""
Download ERA5 data for testing.
Uses CDS API with the key from cra5/api/era5_downloader.py.
Downloads pressure-level and single-level data for 2024-06-01T00:00:00.
"""
import os
import sys
import cdsapi
...
def download_single():
    outfile = os.path.join(DATA_DIR, f'{TIMESTAMP}_single.nc')
    if os.path.exists(outfile) and os.path.getsize(outfile) > 100_000:
        print(f"Single file already exists: {outfile} ({os.path.getsize(outfile)} bytes)")
        return

    print(f"Downloading single level data for {TIMESTAMP}...")
    ...
    tmp_file = outfile + '.tmp'
    client.retrieve('reanalysis-era5-single-levels', request, tmp_file)

    # Handle zip files - ERA5 single-level may contain multiple nc files
    # (e.g. reanalysis vars and forecast vars like tp are in separate files)
    if zipfile.is_zipfile(tmp_file):
        print("Downloaded file is a zip, extracting and merging...")
        import xarray as xr
        with zipfile.ZipFile(tmp_file, 'r') as zf:
            nc_files = [f for f in zf.namelist() if f.endswith('.nc')]
            print(f"  Found {len(nc_files)} nc files in zip: {nc_files}")
            zf.extractall(DATA_DIR, members=nc_files)
            extracted_paths = [os.path.join(DATA_DIR, f) for f in nc_files]
        if len(extracted_paths) == 1:
            os.rename(extracted_paths[0], outfile)
        else:
            ...
            for p in extracted_paths:
                if os.path.exists(p) and p != outfile:
                    os.remove(p)
        os.remove(tmp_file)
    else:
        os.rename(tmp_file, outfile)
