"""
Test LIC-TCM model on ERA5 dataset.
Data processing follows DCAE/test_era5.py: split 268 channels into groups of 3,
apply minmax normalization per group, then feed to the 3-channel LIC-TCM model.
...
Usage:
  ...
  python test_era5.py --ckpt_dir /path/to/checkpoints

  # With actual compress/decompress
  python test_era5.py --ckpt_dir /path/to/checkpoints --compress
"""
...
# Import LIC-TCM model from local models/ directory
...
# Checkpoint configs for LIC-TCM (N=64 for all except mse_lambda_0.05 which is N=128)
CHECKPOINT_CONFIGS = [
    ...
    {'metric': 'mse', 'lmbda': '0.05_large', 'filename': 'mse_lambda_0.05.pth.tar', 'N': 128},
]
...
def find_checkpoints(ckpt_dir):
    ...
    for cfg in CHECKPOINT_CONFIGS:
        fpath = ckpt_dir / cfg['filename']
        if fpath.exists():
            configs.append({
                'metric': cfg['metric'],
                'lmbda': cfg['lmbda'],
                'checkpoint': str(fpath),
                'model_id': f"LICTCM_{cfg['metric']}_lmbda{cfg['lmbda']}",
                'N': cfg['N'],
            })
    # Also pick up any unknown .pth.tar files
    known_names = {c['filename'] for c in CHECKPOINT_CONFIGS}
...
# ─── Main ───
def parse_args():
    parser = argparse.ArgumentParser(description='Test LIC-TCM on ERA5 dataset')
    parser.add_argument('--data_root', type=str,
                        default='/data/run01/scxj523/zsh/project/AIForCompression/Data/ERA5/2024')
    parser.add_argument('--output_dir', type=str,
                        default=os.path.join(os.path.dirname(__file__), 'results_era5'))
    parser.add_argument('--gpu', type=str, default='0')
    parser.add_argument('--checkpoint', type=str, default=None,
                        help='Path to a single LIC-TCM checkpoint (.pth.tar)')
...
def main():
    ...
    for i, ckpt_cfg in enumerate(ckpt_list):
        ...
        net = None
        try:
            net = load_lictcm(ckpt_cfg['checkpoint'], device, N=ckpt_cfg.get('N', 64))
