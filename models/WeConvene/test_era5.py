"""
Test WeConvene model on ERA5 dataset.
Data processing follows DCAE/test_era5.py: split 268 channels into groups of 3,
apply minmax normalization per group, then feed to the 3-channel WeConvene model.
...
# Import WeConvene model from local model/ directory
...
# Checkpoint configs for ECCV WeConvene (actual filenames in checkpoints/ECCV/)
CHECKPOINT_CONFIGS = [
    ...
    for cfg in CHECKPOINT_CONFIGS:
        ...
        if fpath.exists():
            configs.append({
                ...
                'model_id': f"WeConvene_{cfg['metric']}_lmbda{cfg['lmbda']}",
            })
...
# ─── Main ───
def parse_args():
    parser = argparse.ArgumentParser(description='Test WeConvene on ERA5 dataset')
    ...
    parser.add_argument('--checkpoint', type=str, default=None,
                        help='Path to a single WeConvene checkpoint (.pth.tar)')
...
def main():
    ...
    for i, ckpt_cfg in enumerate(ckpt_list):
        ...
        net = None
        try:
            net = load_weconvene(ckpt_cfg['checkpoint'], device)
